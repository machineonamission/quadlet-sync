...
when configured properly, github can ping your server (via webhook) to instantly pull and sync your quadlet setup on
push!

## a few notes

- this repo essentially "takes over" your `/etc/containers/systemd` dir, as rsync will delete anything not in the repo
  in that dir. if you have anything in there, back it up, and move it to your git repo.
- sync events do NOT restart containers (on purpose!). manually restart via systemctl or fedora cockpit
- i have only tested running this as root. no clue if it'll work for non-root.
...
- put `http://[YOUR PUBLIC INSTANCE]/hooks/sync-quadlets` as the payload url
    - replace `[YOUR PUBLIC INSTANCE]` with however you access it (public IP, url, cloudflare tunnel)
        - keep in mind that cloudflare tunnels ignore ports. if you set a cloudflare tunnel at `website.com` to point to
          `localhost:9191`, DO NOT PUT `http://website.com:9191/hooks/sync-quadlets` as the payload url, just put
          `http://website.com/hooks/sync-quadlets`
...
if your webhook was set up right, GitHub should have just pinged it and triggered syncing.